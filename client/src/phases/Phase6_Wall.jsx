...
export const Phase6_UI = ({ currentStage, onSend, inputValue, setInputValue, isGuest, onBack }) => {
  ...
  return (
    ...
    >
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '3rem', color: '#FFD700', textShadow: '0 0 10px gold', margin: '20px' }}>
        MESSAGE WALL
      </h2>
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        <input 
          ...
          disabled={isGuest}
          className="phase6-input"
        />
        <button onClick={onSend} disabled={isGuest} className="phase6-btn" style={{ opacity: isGuest ? 0.5 : 1 }}>
          SEND
        </button>
      </div>
      <button onClick={onBack} className="back-button" style={{ position: 'static', marginTop: '20px', transform: 'none' }}>
        BACK TO MENU
      </button>
    </div>
  );
